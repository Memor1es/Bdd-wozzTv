		...
		if IsControlJustPressed(0, 51) and not onMission and IsPedInAnyVehicle(PlayerPedId()) then
			...
			if IsMissionVehicle(GetEntityModel(veh)) then
				...
				if time >= 0 and time <= TimeToSeconds(5, 30, 0) then
					...
					ShowMPMessage("Cambriolage", "Trouve une ~r~maison ~s~à cambrioler.", 3500)
					--ShowSubtitle("Find a ~r~house ~s~ to rob")
				else
					DisplayHelpText("Les cambriolages ne peuvent se faire qu'entre 0:00 et 5:30.")
				end
			end
		end
		...
		if onMission then		
			for k,door in pairs(doors) do
				...
				if Vdist(coords, door.coords.x, door.coords.y, door.coords.z) < 2 then
					DisplayHelpText("Appuie sur ~INPUT_CONTEXT~ pour entrer dans cette maison.")
					...
					if IsControlJustPressed(0, 51) then
						...
						ShowSubtitle("Tu es dans la maison maintenant, fais attention à ne pas faire trop de bruit.")
					end
				end
			end
		end
