	...
	if DoesEntityExist(ped) and not IsEntityDead(ped) then
		if IsPedOnFoot(ped) then
			if IsPlayerAbleToVape == false then
				...
				ShowNotification("~c~Tu as ~b~commencé ~c~à utilisé la vape.")
				Wait(Config.HelpTextLength)
				TriggerEvent("Vape:HelpFadeOut", 0)
			else
				ShowNotification("~r~Tu es déjà en train de vaper.")
			end
		else
			ShowNotification("~r~Tu ne peux pas faire ça au volant.")
		end
	else
		ShowNotification("~r~Tu ne peux pas faire ça en étant mort.")
	end
end)
...
AddEventHandler("Vape:StopVaping", function(source)
	if IsPlayerAbleToVape == true then
		...
		ShowNotification("~c~Tu as ~r~arrêté ~c~d\'utiliser la vape.")
	else
		ShowNotification("~r~Tu n\'es pas en train de tenir la vape.")
	end
end)
AddEventHandler("Vape:Drag", function()
	if IsPlayerAbleToVape then
		...
		ShowNotification("~r~Tu as besoin d\'avoir une vape pour faire ça.")
	end
end)
	...
	while true do
		if IsPlayerAbleToVape then
			if DisplayText then
				local ped = GetPlayerPed(-1)
				local pedPos = GetEntityCoords(ped)
				DrawText3d(pedPos.x, pedPos.y, pedPos.z+1.20, "~c~Appuie sur ~b~\"E\"~c~ pour tirer une barre.")
				DrawText3d(pedPos.x, pedPos.y, pedPos.z+1.08, "~c~Appuie sur ~b~\"G\"~c~ pour reset la vape.")
			end
		end
	  Wait(1)
	end
