...
AddEventHandler("player:giveItem", function(item, name, qty, target)
    ...
    MySQL.Async.fetchScalar("SELECT SUM(quantity) FROM user_inventory WHERE user_id = @username", { ['@username'] = targetid }, function(total)
        if ( qty < 55) then
            ...
            TriggerClientEvent("es_freeroam:notify", target, "CHAR_SOCIAL_CLUB", 1, "Inventaire", false, "Vous venez de recevoir " .. qty .. " " .. name)
            TriggerClientEvent("es_freeroam:notify", mysource, "CHAR_SOCIAL_CLUB", 1, "Inventaire", false, "Vous venez de donner " .. qty .. " " .. name)
        else
            TriggerClientEvent("es_freeroam:notify", mysource, "CHAR_SOCIAL_CLUB", 1, "Inventaire", false, "quantité trop grande pour l'inventaire du joueur " .. qty .. " " .. name)
        end
    end)
end)
    ...
    MySQL.Async.fetchScalar("SELECT SUM(quantity) FROM user_inventory WHERE user_id = @username", { ['@username'] = player }, function(total)
        if menus[item] ~= nil then
            if (qty > 0) then
                ...
                TriggerClientEvent("player:looseItem", mysource, item, qty)
                TriggerClientEvent("es_freeroam:notify", mysource, "CHAR_SOCIAL_CLUB", 1, "Inventaire", false, "Vous utilisez " .. qty .. " " .. name)
            end
        else
            TriggerClientEvent("es_freeroam:notify", mysource, "CHAR_SOCIAL_CLUB", 1, "Inventaire", false, "Vous ne pouvez pas utiliser cet item")
        end
    end)
end)
