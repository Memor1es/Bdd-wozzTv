...
-- System Infos
PerformHttpRequest(DiscordWebhookSystemInfos, function(Error, Content, Head) end, 'POST', json.encode({username = SystemName, content = '**Démarrage du serveur et des webhooks**'}), { ['Content-Type'] = 'application/json' })
...
AddEventHandler('DiscordBot:ToDiscord', function(WebHook, Name, Message, Image, Source, TTS, FromChatResource)
	...
	elseif WebHook:lower() == 'drugs' then
		WebHook = DiscordWebhookDrugs
	elseif WebHook:lower() == 'salary' then
		WebHook = DiscordWebhookSalary
