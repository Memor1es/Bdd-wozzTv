...
AddEventHandler('player_state:change', function(source, calories, waterdrops, wc)
    ...
    MySQL.Async.fetchAll("SELECT * FROM users WHERE identifier = @name", {['@name'] = identifiers[1]}, function(result)
        if(result[1])then
            ...
            MySQL.Async.execute("UPDATE users SET `food`=@foodvalue, `water`=@watervalue  WHERE identifier = @identifier",{ ['@foodvalue'] = new_food, ['@watervalue'] = new_water, ['@identifier'] = identifiers[1]}, function()
                if calories > 0 then
                ...
                TriggerClientEvent("player_state:set", currentSource, {food = new_food, water = new_water})
            end)
        end
    end)   
end)
...
AddEventHandler('player_state:check', function()
    ...
    MySQL.Async.fetchAll("SELECT * FROM users WHERE identifier = @name", {['@name'] = identifiers[1]}, function(result)
        if(result[1])then
            if (tonumber(result[1].food) >= 1) and (tonumber(result[1].water) >= 1) then
                TriggerClientEvent('player_state:set', currentSource, {food = result[1].food, water = result[1].water})
                if (tonumber(result[1].food) <= 0) or (tonumber(result[1].water) <= 0)  then
