function closures_event_server()
    local listMissions = {}
    local listPersonnelActive = {}
    local acceptMulti = false
    local preFixEventName = 'event'
    ...
    function addMission(source, position, type)
        ...
        if sMission == nil then
          if getNbPerosnnelDispo() == 0 and type == "1 personne" then
            TriggerClientEvent('event:callAutoevent', source, position, type)
    ...
    function personelAcceptMission(source, missionId)
        ...
        elseif #sMission.acceptBy ~= 0  and not acceptMulti then
            ...
            TriggerClientEvent('event:MissionAccept', source, sMission)
            ...
            for k, v in pairs(mission.acceptBy) do
                if v == personnelId then
                    ...
                    if #mission.acceptBy == 0 then
                        ...
                        TriggerClientEvent('event:callStatus', mission.id, 2)
    ...
    RegisterServerEvent('event:PayAutomaticevent')
    AddEventHandler('event:PayAutomaticevent', function (price)
      TriggerEvent('es:getPlayerFromId', source, function(user)
        user.removeMoney(price)
      end)
    end)

    RegisterServerEvent('event:takeService')
    AddEventHandler('event:takeService', function ()
        addPersonelService(source)
        notifyPersonnelChange()
    end)

    RegisterServerEvent('event:endService')
    AddEventHandler('event:endService', function ()
        removeMeccano(source)
        removePersonelService(source)
    end)

    RegisterServerEvent('event:requestMission')
    AddEventHandler('event:requestMission', function ()
        notifyMissionChange(source)
    end)

    RegisterServerEvent('event:requestPersonnel')
    AddEventHandler('event:requestPersonnel', function ()
        notifyPersonnelChange(source)
    end)

    RegisterServerEvent('event:Call')
    AddEventHandler('event:Call', function (posX,posY,posZ,type)
        addMission(source, {posX, posY, posZ}, type)
    end)

    RegisterServerEvent('event:CallCancel')
    AddEventHandler('event:CallCancel', function ()
        removeClient(source)
    end)

    RegisterServerEvent('event:AcceptMission')
    AddEventHandler('event:AcceptMission', function (id)
        personelAcceptMission(source, id)
    end)

    RegisterServerEvent('event:FinishMission')
    AddEventHandler('event:FinishMission', function (id)
        closeMission(source, id)
    end)

    RegisterServerEvent('event:cancelCall')
    AddEventHandler('event:cancelCall', function ()
        removeClient(source)
    end)
    ...
    RegisterServerEvent("Syncevent")
    AddEventHandler('Syncevent', function(inputText)
        TriggerClientEvent('Displayevent', -1, inputText)
    end)


end

closures_event_server()
