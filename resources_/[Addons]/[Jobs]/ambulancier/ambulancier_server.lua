function closures_ambulancier_server()
    ...
    function personelAcceptMissionAmbulancier(source, missionId)
        ...
        elseif #sMission.acceptBy ~= 0  and not acceptMultiAmbulancier then 
            ...
            TriggerClientEvent('ambulancier:MissionAccept', source, sMission)
            ...
            for k, v in pairs(mission.acceptBy) do 
                if v == personnelId then
                    ...
                    if #mission.acceptBy == 0 then
                        ...
                        TriggerClientEvent('ambulancier:callStatus', mission.id, 2)
    ...
    RegisterServerEvent('ambulancier:takeService')
    AddEventHandler('ambulancier:takeService', function ()
        addPersonelServiceAmbulancier(source)
        notifyPersonnelAmbulancierChange()
    end)

    RegisterServerEvent('ambulancier:endService')
    AddEventHandler('ambulancier:endService', function ()
        removePersonnelAmbulancier(source)
        removePersonelServiceAmbulancier(source)
    end)

    RegisterServerEvent('ambulancier:requestMission')
    AddEventHandler('ambulancier:requestMission', function ()
        notifyMissionChangeAmbulancier(source)
    end)

    RegisterServerEvent('ambulancier:requestPersonnel')
    AddEventHandler('ambulancier:requestPersonnel', function ()
        notifyPersonnelAmbulancierChange(source)
    end)

    RegisterServerEvent('ambulancier:Call')
    AddEventHandler('ambulancier:Call', function (type, positionBackUp)
        addMissionAmbulancier(source, type, positionBackUp)
    end)

    RegisterServerEvent('ambulancier:CallCancel')
    AddEventHandler('ambulancier:CallCancel', function ()
        removeClientAmbulancier(source)
    end)

    RegisterServerEvent('ambulancier:AcceptMission')
    AddEventHandler('ambulancier:AcceptMission', function (id)
        personelAcceptMissionAmbulancier(source, id)
    end)

    RegisterServerEvent('ambulancier:FinishMission')
    AddEventHandler('ambulancier:FinishMission', function (id)
        closeMissionAmbulancier(source, id)
    end)

    RegisterServerEvent('ambulancier:cancelCall')
    AddEventHandler('ambulancier:cancelCall', function ()
        removeClientAmbulancier(source)
    end)
    ...
    function()
        local player = source
        TriggerEvent('player_state:change',player,50 , 50 , 50)
    end)
    ...
    RegisterServerEvent("SyncAmbulancier")
    AddEventHandler('SyncAmbulancier', function(inputText)
        TriggerClientEvent('DisplayAmbulancier', -1, inputText)
    end)
end
