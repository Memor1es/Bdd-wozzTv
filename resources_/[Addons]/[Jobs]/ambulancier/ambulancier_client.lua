...
ambulance_nbAmbulanceInService = 0
...
function getIsInServiceAmbulancier()

	return ambulancierIsInService
...
local function toogleServiceAmbulancier()
    ...
    if ambulancierIsInService then
        ...
        if(GetEntityModel(GetPlayerPed(-1)) == hashSkin) then
			...
			SetPedComponentVariation(GetPlayerPed(-1), 4, 25, 2, 2)
        ...
        TriggerEvent('Ambulancier:Advert')
...
RegisterNetEvent("Ambulancier:Advert")

AddEventHandler("Ambulancier:Advert",function()



		TriggerServerEvent('SyncAmbulancier', "Disponible")
...
RegisterNetEvent('DisplayAmbulancier')

AddEventHandler('DisplayAmbulancier',function(inputText)
...
SetNotificationMessage("CHAR_CALL911", "CHAR_CALL911", true, 1, "~y~Ambulancier En Service~s~", "");
...
local function gestionServiceAmbulancier()
    ...
    if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), ambulancier_blips["Urgences"].x, ambulancier_blips["Urgences"].y, ambulancier_blips["Urgences"].z, true) <= ambulancier_blips["Urgences"].distanceMarker+1 then
        DrawMarker(27, ambulancier_blips["Urgences"].x, ambulancier_blips["Urgences"].y, ambulancier_blips["Urgences"].z, 0, 0, 0, 0, 0, 0, 2.001, 2.0001, 0.5001, 0, 155, 255, 200, 0, 0, 0, 0)
        ClearPrints()
        SetTextEntry_2("STRING")
        if ambulancierIsInService then
            AddTextComponentString("~g~ENTREE~s~ pour quitter ton ~b~service")
        else
            AddTextComponentString("~g~ENTREE~s~ pour prendre ton ~b~service")
        end
...
RegisterNetEvent('ambulancier:callServiceCustom')

AddEventHandler('ambulancier:callServiceCustom',function(data)
