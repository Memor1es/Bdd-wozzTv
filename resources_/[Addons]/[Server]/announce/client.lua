...
AddEventHandler("Scaleform:Announce", function(type, time, fontsize, announcement)
	...
	if type == 1 then
		...
		AnnouncementScaleform:CallFunction("SHOW_SHARD_MIDSIZED_MESSAGE", "Annonce RainbowLand", announcement, 2, true, true)
...
RegisterNetEvent("Scaleform:Announce2")

AddEventHandler("Scaleform:Announce2", function(title, type, time, fontsize, announcement)
	...
	if type == 1 then
		...
		AnnouncementScaleform:CallFunction("SHOW_SHARD_MIDSIZED_MESSAGE", title, announcement, 2, true, true)
