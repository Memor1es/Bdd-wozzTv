		...
		if isHolding then
			...
			if IsControlJustPressed(0, 51) then	
				...
				if isInHouse then
					...
					if Vdist(markerCoords, coords) < 3 then
						ShowSubtitle("Objet ranger, retourne à l'intérieur pour voler d'autres objets.")
						
						-- send to server
						TriggerServerEvent("avanyacamb:collected", currentItem.id, currentItem.house)
			...
			for _,pickup in pairs(pickups) do
				...
				if onMission and Vdist(coords, GetEntityCoords(pickup.prop)) < 2 and not isHolding then
					DisplayHelpText("Appuie sur ~INPUT_CONTEXT~ pour voler cet objet")
					...
					if IsControlJustPressed(0, 51) then
						...
						ShowSubtitle("Tu as pris un objet, ramène le au Van à l'extérieur.")
