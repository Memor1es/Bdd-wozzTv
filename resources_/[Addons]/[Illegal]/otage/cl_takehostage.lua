...
function takeHostage()
	...
	if not canTakeHostage then 
		drawNativeNotification("Tu as besoin d\'un pistolet avec munitions!")
	end
	...
	if not holdingHostageInProgress and canTakeHostage then		
		...
		if closestPlayer ~= -1 then
			...
			drawNativeNotification("Aucun joueur à proximité")
		end 
	end
...
Citizen.CreateThread(function()
	while true do 
		if holdingHostage then
			...
			DrawText3D(playerCoords.x,playerCoords.y,playerCoords.z,"Appuie sur [G] pour annuler, [H] pour tuer")
