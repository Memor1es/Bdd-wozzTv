...
--		if IsNearDistribCabine() then
--			TriggerEvent('taxi:callServiceCustom')
--			end
--        end
--	end
--end)

function IsNearDistribCabine()

	local ped = GetPlayerPed(-1)
	local playerPos = GetEntityCoords(ped)

	for index,value in ipairs(FixePhone1) do
		local obj = GetClosestObjectOfType(playerPos.x, playerPos.y, playerPos.z, 1.5, GetHashKey(value), false, false, false)
		if obj ~= 0 then
			return true
	    else
		Citizen.Wait(10)
