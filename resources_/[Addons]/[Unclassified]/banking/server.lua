...
function bankBalance(player)
    return tonumber(MySQL.Sync.fetchScalar("SELECT bank FROM users WHERE identifier = @name", {['@name'] = player}))
end
    ...
    TriggerEvent('es:getPlayerFromId', mysource, function(user)
        ...
        if(string.len(rounded) >= 10) then
            ...
            if(tonumber(rounded) <= tonumber(bankbalance)) then
                ...
                TriggerClientEvent("banking:updateBalance", mysource, user.getBank())
                if (amount > 1) and (group == "user") then
                    TriggerEvent('DiscordBot:ToDiscord', 'salary', GetPlayerName(_source), GetPlayerName(_source).." vient de retirer : "..amount.."$\nGroupe du joueur : "..group.."\n CHEAT POTENTIEL !", 'steam', 0, false, false)
                end
