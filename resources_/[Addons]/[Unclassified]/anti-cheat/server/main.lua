...
TAC.LogBanToDiscord = function (data)
    ...
    local discordInfo = {
        ...
        ["footer"] = {
            ["text"] = 'Avanya A-C | ' .. TAC.Version
        }
    }

    PerformHttpRequest(TAC.Config.Webhook, function(err, text, headers) end, 'POST', json.encode({ username = 'Avanya A-C', embeds = { discordInfo } }), { ['Content-Type'] = 'application/json' })
end
