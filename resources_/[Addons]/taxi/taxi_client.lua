...
local TEXT = {
  ...
  PrendreService = '~INPUT_PICKUP~ ~g~Prendre son service de taxi',

  QuitterService = '~INPUT_PICKUP~ ~r~Quitter son service de taxi',
...
-- local chestPos = { ['x'] = 896.18, ['y'] = -144.86, ['z'] = 76.78 }
...
local function IAgotoPlayer(type)
  ...
  DecorSetBool(IAcar, "Arcadia", true)
...
function getIsInServiceTaxi()

return inServiceTaxi
  ...
  for _, pos in pairs(coords) do
    ...
    if pos.type == type then
      ...
      if vehi == 0 then
		...
		DecorSetBool(myVehiculeEntity, "Arcadia", true)
...
local function toogleService()
  ...
  inServiceTaxi = not inServiceTaxi

  if inServiceTaxi then
...
AddEventHandler("Taxi",function()
		...
		--TriggerServerEvent('SyncTaxi', "Disponible")
...
SetNotificationMessage("CHAR_TAXI", "CHAR_TAXI", true, 1, "~y~Taxi En Service~s~", "");
	...
	for _, coordData in pairs(coords) do
		...
		if dist <= 20 then
			...
			if inServiceTaxi then
				...
				DrawMarker(25, pos.x, pos.y, pos.z - 0.97, 0, 0, 0, 0, 0, 0, 1.0, 1.0, 1.5, 255, 0, 0, 50, 0, 0, 0, 0)

			else

				DrawMarker(25, pos.x, pos.y, pos.z - 0.97, 0, 0, 0, 0, 0, 0, 1.0, 1.0, 1.5, 0, 255, 0, 50, 0, 0, 0, 0)
		...
		if inServiceTaxi then
			...
			if dist <= 20 then
				...
				DrawMarker(25, pos.x, pos.y, pos.z - 0.97, 0, 0, 0, 0, 0, 0, 1.0, 1.0, 1.5, 255, 255, 255, 50, 0, 0, 0, 0)
...
Citizen.CreateThread(function()
  ...
  while true do
	...
	--		if(Vdist(chestPos.x, chestPos.y, chestPos.z, pos.x, pos.y, pos.z) < 20.0) then

	--			DrawMarker(1, chestPos.x, chestPos.y, chestPos.z - 1, 0, 0, 0, 0, 0, 0, 1.0, 1.0, 1.0, 255, 102, 0, 55, 0,0, 0,0)

	--			if(Vdist(chestPos.x, chestPos.y, chestPos.z, pos.x, pos.y, pos.z) < 1.5) then

	--				DisplayHelpText("Coffre Taxi ? ~INPUT_CONTEXT~")
	...
	--				if MenuChest.isOpen then

	--					MenuChest.draw()

	--					MenuChest.keyControl()
...
--	MenuChest.isOpen = not MenuChest.isOpen
...
RegisterNetEvent('taxi-recrue:drawMarker')

AddEventHandler('taxi-recrue:drawMarker', function (boolean)
...
RegisterNetEvent('taxi-patron:drawMarker')

AddEventHandler('taxi-patron:drawMarker', function (boolean)
...
RegisterNetEvent('taxi-recrue:drawBlips')

AddEventHandler('taxi-recrue:drawBlips', function ()

end)

RegisterNetEvent('taxi-recrue:marker')

AddEventHandler('taxi-recrue:marker', function ()

end)



RegisterNetEvent('taxi-patron:drawBlips')

AddEventHandler('taxi-patron:drawBlips', function ()

end)

RegisterNetEvent('taxi-patron:marker')

AddEventHandler('taxi-patron:marker', function ()
...
RegisterNetEvent('taxi-recrue:deleteBlips')

AddEventHandler('taxi-recrue:deleteBlips', function ()
...
RegisterNetEvent('taxi-patron:deleteBlips')

AddEventHandler('taxi-patron:deleteBlips', function ()
...
--RegisterNetEvent("taxi:moneyAdd")

--AddEventHandler("taxi:moneyAdd", function(data)
...
--		TriggerServerEvent('taxi:addMoneyServer', value, 0)
...
--RegisterNetEvent("taxi:moneyRem")

--AddEventHandler("taxi:moneyRem", function(data)
...
--		TriggerServerEvent('taxi:removeMoneyServer', value, 0)
...
--RegisterNetEvent("taxi:moneyShow")

--AddEventHandler("taxi:moneyShow", function(data)

--

--	TriggerServerEvent('taxi:getMoneyInfos')
...
AddEventHandler('taxi:openMenu', function()
  ...
  if inServiceTaxi then
    ...
    showMessageInformation("~r~Vous devez etre en service pour accedez au menu")
...
RegisterNetEvent('taxi-recrue:openMenu')

AddEventHandler('taxi-recrue:openMenu', function()
...
RegisterNetEvent('taxi-patron:openMenu')

AddEventHandler('taxi-patron:openMenu', function()
