		...
		if result[1] ~= nil then
			if result[1].timeleft > 0 then
				if result[1].cellule ~= nil then
					TriggerClientEvent("avanyajail:JailInStation", source,result[1].cellule,result[1].timeleft)
				end
...
RegisterServerEvent("avanyajail:PutInJail")
AddEventHandler("avanyajail:PutInJail", function(playerid, jail, jailtime)
	--calculate jailtime
	...
	if jail ~= nil then
			print("[AvanyaJAIL] Mettre en prison en ".. jail.." :".. GetPlayerName(playerid).. " pour ".. jailtime .." secs - commande entree par ".. GetPlayerName(source))
			...
			TriggerClientEvent("avanyajail:JailInStation", playerid,jail,jailtime)
	end
end)

RegisterServerEvent("avanyajail:UnJailplayer")
AddEventHandler("avanyajail:UnJailplayer", function(playerid)
	local identifier = GetPlayerIdentifiers(playerid)[1]
	if GetPlayerName(playerid) ~= nil then
		TriggerClientEvent("avanyajail:UnJail", playerid)
...
RegisterServerEvent("avanyajail:UnJailplayer2")
AddEventHandler("avanyajail:UnJailplayer2", function()
	local identifier = GetPlayerIdentifiers(source)[1]
	if GetPlayerName(source) ~= nil then
		TriggerClientEvent("avanyajail:UnJail", source)
		-- MySQL.Async.execute("INSERT INTO jail (identifier,J_Time, isjailed) VALUES (@Identifier,@J_Time,@isjailed) ON DUPLICATE KEY UPDATE Identifier=identifier,isjailed=@isjailed,J_Time=@J_Time", {['@identifier'] = identifier, ['@J_Time'] = os.date('%Y-%m-%d %H:%M:%S'),['@isjailed'] = false})
		-- MySQL.Async.execute("UPDATE jail SET `isjailed` = false where identifier = @identifier", {['@identifier'] = identifier)
		MySQL.Async.execute("DELETE FROM jail WHERE identifier=identifier", {['@identifier'] = identifier})
	end
