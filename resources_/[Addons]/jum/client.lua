...
function Notify(icon, title, desc, text)
	...
	while true do
		...
		if binoculars then
			...
			while binoculars and not IsEntityDead(lPed) and (GetVehiclePedIsIn(lPed) == vehicle) and true do
				...
				--TriggerEvent("es_freeroam:notify", "CHAR_SOCIAL_CLUB", "Jumelles", "", "Pour ~r~desactiver~s~ les jumelles appuyez sur ~r~retour~s~ ou ~r~clique droit~s~")
				TriggerEvent("es_freeroam:notify", "CHAR_SOCIAL_CLUB", 1, "Jumelles", false, "Pour ~r~désactiver~s~ les jumelles, appuyez sur ~r~retour~s~ ou ~r~ clique droit~s~.")
