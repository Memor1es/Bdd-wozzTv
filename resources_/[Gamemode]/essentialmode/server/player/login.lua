...
function LoadUser(identifier, source, new, licenseNotRequired)
	...
	db.retrieveUser(identifier, function(user)
		if user.license or licenseNotRequired then
			Users[source] = CreatePlayer(source, user.permission_level, user.money, user.bank, user.identifier, user.license, user.group, user.dirtymoney, user.roles or "")
			...
			if(settings.defaultSettings.showBank)then
			Users[source].setBank(user.bank)
			end
			if(settings.defaultSettings.dirtyMoneyShow)then
			Users[source].setDirtyMoney(user.dirtymoney)
			end
...
function LoadLicenseUser(license, source, id)
	...
	db.retrieveLicensedUser(license, function(user)
		Users[source] = CreatePlayer(source, user.permission_level, user.money, user.bank, user.identifier, user.license, user.group, user.dirtymoney)
	...
	SetTimeout(60000, function()
		Citizen.CreateThread(function()
			for k,v in pairs(Users)do
				if Users[k] ~= nil then
					db.updateUser(v.get('identifier'), {money = v.getMoney(), bank = v.getBank(), dirtymoney = v.getDirtyMoney()}, function()end)
				end
			end
