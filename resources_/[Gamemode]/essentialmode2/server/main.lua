...
settings.defaultSettings = {
	...
	['startingCash'] = 1500, -- FIRST JOIN, MONEY ?
	['startingBank'] = 5500, -- FIRST JOIN, MONEY IN BANK ?
	...
	['showbank'] = false, -- SHOW BANK BALANCE ?
...
AddEventHandler('playerDropped', function(reason)
	...
	if(Users[Source])then
		...
		if(settings.defaultSettings.tpLastPos)then
			MySQL.Async.execute("UPDATE users SET `money`=@valmoney, `bank`=@valbank, `dirtymoney`=@valdirtymoney, `lastPosition`=@vallastposition WHERE identifier = @identifier",
			{['@valmoney'] = Users[Source].getMoney(), ['@valbank'] = Users[Source].getBank(), ['@valdirtymoney'] = Users[Source].getDirtyMoney(), ['vallastposition'] = json.encode(Users[Source].getCoords()), ['@identifier'] = Users[Source].getIdentifier()})
		else
			MySQL.Async.execute("UPDATE users SET `money`=@valmoney, `bank`=@valbank, `dirtymoney`=@valdirtymoney WHERE identifier = @identifier",
			{['@valmoney'] = Users[Source].getMoney(), ['@valbank'] = Users[Source].getBank(), ['@valdirtymoney'] = Users[Source].getDirtyMoney(), ['@identifier'] = Users[Source].getIdentifier()})
		end
		Users[Source] = nil
	end
end)
