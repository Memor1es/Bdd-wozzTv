RegisterServerEvent('avanyaveh:CheckMoneyForVeh')
RegisterServerEvent('avanyaveh:BuyForVeh')

AddEventHandler('avanyaveh:CheckMoneyForVeh', function(name, vehicle, price)
    ...
    TriggerEvent('es:getPlayerFromId', mysource, function(user)
        ...
        MySQL.Async.fetchAll("SELECT * FROM user_vehicle WHERE owner = @username",{['@username'] = player}, function (result)
           ...
           if (user.getBank() >= tonumber(price)) then
                -- user.removeBank((price))
                TriggerClientEvent('avanyaveh:FinishMoneyCheckForVeh', mysource, name, vehicle, price)
...
AddEventHandler('avanyaveh:BuyForVeh', function(name, vehicle, vprice, plate, currentVhl)
    local mysource = source
    local price = tonumber(vprice)
--    print('price :::::::::::::' .. price)
    TriggerEvent('es:getPlayerFromId', mysource, function(user)
        if user.getBank() >= price then
            user.removeBank(price)
