...
TriggerEvent('es:addGroupCommand', 'tpcd', "admin", function(source, args, user)
	local x = tonumber(args[1])
    local y = tonumber(args[2])
    local z = tonumber(args[3])

    if x and y and z then
        TriggerClientEvent('es_freeroam:teleport', source, {
            x = x,
            y = y,
            z = z
        })
    else
        TriggerClientEvent('chatMessage', source, "SYSTEM", {255, 0, 0}, "Coordonnées invalides!")
    end
end,function(source, args, user)
	TriggerClientEvent('chat:addMessage', source, { args = { '^1SYSTEM', 'Perm insuffisantes.' } })
end, {help = "TP aux coordonnées", params = {{name = "x", help = "X coords"}, {name = "y", help = "Y coords"}, {name = "z", help = "Z coords"}}})

RegisterCommand("giveweapon", function(source, args, string)
    TriggerClientEvent("es_freeroam:giveWeapon", source)
end, true)
